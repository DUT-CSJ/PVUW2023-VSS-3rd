...
# --------------------------------------------------------
_base_ = [
    '../_base_/models/upernet_r50.py', '../_base_/datasets/coco-stuff164k.py',
...
model = dict(
    backbone=dict(
        ...
        with_cp=True,
        out_indices=(0, 1, 2, 3),
        init_cfg=dict(type='Pretrained', checkpoint=pretrained)), #dict(type='Pretrained', checkpoint=pretrained)),
    decode_head=dict(num_classes=171, in_channels=[320, 640, 1280, 2560]),
    auxiliary_head=dict(num_classes=171, in_channels=1280),
...
# optimizer_config = dict(type='Fp16OptimizerHook', loss_scale='dynamic', _delete_=True, grad_clip=dict(max_norm=0.1, norm_type=2))
evaluation = dict(interval=32000, metric='mIoU', save_best='mIoU')
# fp16 = dict()
